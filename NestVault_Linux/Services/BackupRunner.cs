...
public partial class BackupRunner : ObservableObject
{
    ...
    public async Task RunAsync(BackupProfile profile)
    {
        ...
        var serverPaths = scannedFiles.Select(f =>
        {
            ...
            var rel = f.Path.StartsWith(source) ? f.Path[source.Length..] : "/" + System.IO.Path.GetFileName(f.Path);
        ...
        var hashTasks = slowEntries.Select(async entry =>
        {
            ...
            try
            {
                ...
                CurrentFile = System.IO.Path.GetFileName(entry.Path);
            }
        ...
        var execTasks = workItems.Select(async item =>
        {
            ...
            try
            {
                ...
                CurrentFile = System.IO.Path.GetFileName(item.Path);
            }
            catch (OperationCanceledException) { throw; }
            catch (Exception ex)
            {
    ...
    {
        var versionKey = DateTime.Now.ToString("yyyy-MM-ddTHH:mm:ss");
...
public class LocalHashCache
{
    ...
    public LocalHashCache(string label)
    {
        ...
        _filePath = System.IO.Path.Combine(dir, $"{safe}_hashcache.json");
    }
